        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h2 class="text-center">Laporan Mengikut Tarikh / Bulanan Fail Am</h2>
                </div>
                <div class="card-body">
                    <!-- Centered content using Bootstrap classes and custom CSS -->
                    <div class="centered-content mb-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input file-type" type="radio" name="fail" id="baru">
                            <label class="form-check-label" for="baru">Fail Baru</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input file-type" type="radio" name="fail" id="tutup">
                            <label class="form-check-label" for="tutup">Fail Tutup</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input file-type" type="radio" name="fail" id="pinjam">
                            <label class="form-check-label" for="pinjam">Fail Pinjam</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input file-type" type="radio" name="fail" id="pulang">
                            <label class="form-check-label" for="pulang">Fail Pulang</label>
                        </div>
                    </div>
                    ...
                    <div class="centered-content mb-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input date-selection" type="radio" name="MB" id="mingguan" onchange="toggleDateFields()">
                            <label class="form-check-label" for="mingguan">Mengikut Tarikh</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input date-selection" type="radio" name="MB" id="bulanan" onchange="toggleDateFields()">
...
<script>
    ...
    function updateYearDropdown() {
        var fileType = document.querySelector('input[name="fail"]:checked')?.id;

        if (fileType) {
            // Fetch available years for the selected file type using AJAX
            $.ajax({
                url: "<?= base_url('/failAm/getAvailableYears') ?>",
                type: "GET",
    ...
    document.getElementById('report-button').addEventListener('click', function (event) {
        ...
        // Check if "Mengikut Tarikh" is selected and validate date fields
        var mingguan = document.getElementById('mingguan');
        var bulanan = document.getElementById('bulanan');
        if (mingguan.checked) {
        ...
        var url = "<?= base_url('/failAm/laporanTkhBlnTbl') ?>?fileType=" + fileType;

        // Add only the selected parameters to the URL
        if (startDate && endDate) {
