...
<div class="container custom-margin">
    <div class="row justify-content-center">
        <div class="d-flex justify-content-end">
            <a href="<?= base_url('failAm/laporanSttsPmnjm') ?>" class="btn btn-danger btn-sm float-end">Kembali</a>
        </div>
        <div class="col-md-12">
            <?php 
                if(session()->getFlashdata('statuspmnjmtblfa'))
                {
                    ?>
                        <div class="alert alert-warning alert-dismissible fade show" role="alert">
                            <?= session()->getFlashdata('statuspmnjmtblfa'); ?>
            ...
            <?php endif; ?>
            <?php if (!empty($failAmPinjamData)): ?>
            <div class = "export-button-wrapper">
                <form method="post" action="<?= site_url('/failAm/laporanSttsPmnjmTblPDF') ?>" id="export-form">
                    ...
                    <textarea name="failAmPinjamData" hidden><?= json_encode($failAmPinjamData) ?></textarea>
                ...
                <form method="post" action="<?= site_url('/failAm/laporanSttsPmnjmTblExcel') ?>" id="export-form">
                    <input type="hidden" name="reportTitle" value="<?= $reportTitle ?>">
                    <input type="hidden" name="dateDetails" value="<?= $dateDetails ?>">
                    <input type="hidden" name="fileType" value="<?= $fileType ?>">
            ...
            <div class="table-responsive">
                <table class="table table-striped" id="table">
                    ...
                    <tbody>
                        <?php if (!empty($failAmPinjamData)): ?>
                            <?php foreach ($failAmPinjamData as $index => $fail): ?>
                                <tr>
                                    <td><?= $index + 1 ?></td>
