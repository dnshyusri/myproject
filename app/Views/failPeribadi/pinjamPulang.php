        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="row mt-2">
                        <div class="col-md-4">
                            <strong>Nama Fail:</strong> <?= $failPeribadi['NAMA'] ?>
                        </div>
                        <div class="col-md-4">
                            <strong>No. Fail:</strong> <?= $failPeribadi['NOFAIL'] ?>
                        </div>
                        <div class="col-md-4">
                            <strong>No. Jilid:</strong> <?= $failPeribadi['NOJILID'] ?>
                        </div>
                    </div>
                </div>

                <div class="card-body">
                    <form class="row g-3" action="<?= base_url('failPeribadi/addPinjamPulang') ?>" method="POST">
                        ...
                        <div class="form-group mb-2">
                            <label>Nama</label>
                            <?php if ($failPeribadiPinjam != null && $failPeribadiPinjam['STATUS2'] == 'DIPINJAM') { ?>
                                <select name="NAMAPENGGUNA" id="NAMAPENGGUNA" class="form-select">
                                    ...
                                    <option value="<?= $failPeribadiPinjam['NAMAPENGGUNA'] ?>" selected><?= $failPeribadiPinjam['NAMAPENGGUNA'] ?></option>
                                </select>
                            <?php } else { ?>
                                <select name="NAMAPENGGUNA" id="NAMAPENGGUNA" class="form-select">
                                    <option value="" disabled selected>Pilih Nama Peminjam</option> <!-- Placeholder -->
                                    ...
                                    foreach ($peminjam as $NamaPeminjam) { ?>
                                        <option value="<?= $NamaPeminjam['user'] ?>" <?= ($failPeribadiPinjam != null && $failPeribadiPinjam['NAMAPENGGUNA'] == $NamaPeminjam['user'])?>>
                                            <?= $NamaPeminjam['user'] ?>
                                        </option>
                                    <?php } ?>
                                </select>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="STATUS2" id="STATUS2" class="form-select" onchange="updateNamaOptions()" required>
                                ...
                                <?php if ($failPeribadiPinjam != null) { ?>
                                    ...
                                    <?php } elseif ($failPeribadiPinjam['STATUS2'] == 'DIPULANG') { ?>
                                    ...
                                    <?php } elseif ($failPeribadiPinjam['STATUS2'] == 'DITUTUP') { ?>
                                        <option value="DIPINJAM" <?= ($failPeribadiPinjam['STATUS2'] == 'DIPINJAM') ? 'selected' : '' ?>>DIPINJAM</option>
